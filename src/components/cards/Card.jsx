    ...
    <div className="card bg-base-100 w-full sm:w-80 md:w-78 shadow-sm overflow-hidden lg:w-96">
      ...
      <div className="card-body p-4 flex flex-col justify-between">
        <div className="space-y-1">
          <h2 className="card-title text-base sm:text-lg">{className}</h2>
          ...
          <p className="text-sm">By trainer: {trainerName}</p>
        ...
        <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          {role === "user" ? (
            ...
            <button className="btn btn-neutral btn-disabled w-full sm:w-auto">
              you can't book only user
            </button>
          )}

          <div className="flex justify-end items-center gap-2">
            <p className="text-gray-700 font-bold text-sm">Total</p>
            <p className="text-amber-600 font-bold text-base">{price} baht</p>
          </div>
        </div>
      </div>
    </div>
